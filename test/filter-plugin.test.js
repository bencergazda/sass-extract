...
const testSets = {
  ...
  mix: [
    ...
    ["numbers and string", {
      filterOptions: {
        only: { types: ["SassNumber"] }, except: { props: ["$number1"] },
      },
      expectedProps: pick(PROPS_ALL, ["$number2"]),
    }],
    ["numbers and string", {
      filterOptions: {
        only: { types: ["SassNumber"] }, except: { types: ["SassNumber"] },
      },
      expectedProps: PROPS_NONE,
    }],
    ["numbers and string", {
      filterOptions: {
        only: { props: ["$number1"] }, except: { props: ["$number1"] },
      },
      expectedProps: PROPS_NONE,
    }]
  ],
...
describe("filter-plugin", () => {
  describe.each(renderFunctions)("%s", (renderFuncType, renderFunc) => {
    describe.each(Object.keys(testSets))('%s', (filterType) => {
      it.each(testSets[filterType])('should include %s', async (shouldInclude, { filterOptions, expectedProps }) => {
        const rendered = await renderFunc({ file: filterPluginFile }, {
          plugins: [{
            plugin: "filter",
